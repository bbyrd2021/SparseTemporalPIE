"""
train_SparseTemporalPIE_v3_jaad.py
====================================
Base training for SparseTemporalPIE v3 on JAAD: IL step=0, 50 epochs.
"""
...
from models.SparseTemporalPIE_v3 import SparseTemporalPIE_v3, load_backbone_weights
from utils.train_val import evaluate_sparse_v3, robust_noisy
...
def train_one_epoch(model, dataloader, optimizer, device, epoch, total_epochs):
    ...
    for step, (f_current, f_context, context_mask, pose_current, pose_context,
               bbox_traj, ctx_feats, labels) in enumerate(dataloader):
        f_current    = f_current.to(device)
        f_context    = f_context.to(device)
        context_mask = context_mask.to(device)
        pose_current = pose_current.to(device)
        pose_context = pose_context.to(device)
        bbox_traj    = bbox_traj.to(device)
        ctx_feats    = ctx_feats.to(device)
        labels       = labels.to(device)

        optimizer.zero_grad()
        logits   = model(f_current, f_context, context_mask, pose_current, pose_context,
                         bbox_traj, ctx_feats)
...
def main(args):
    ...
    train_seq = jaad.generate_data_trajectory_sequence('train', **data_opts)
    val_seq   = jaad.generate_data_trajectory_sequence('val',   **data_opts)
    seq_len   = data_opts['max_size_observe']

    train_seq_data = jaad.get_train_val_data(train_seq, data_type, seq_len, data_opts['seq_overlap_rate'])
    val_seq_data   = jaad.get_train_val_data(val_seq,   data_type, seq_len, data_opts['seq_overlap_rate'])
    ...
    model = SparseTemporalPIE_v3(dropout=0.1).to(device)
    ...
    train_dataset = SparseDataset_v3(train_seq_data, data_opts, step=args.step,
                                     transform=train_transform,
                                     keypoints_dir=args.keypoints_dir,
                                     flip_p=0.5, pose_dropout_p=0.1)
    val_dataset   = SparseDataset_v3(val_seq_data,   data_opts, step=args.step,
    ...
    best_val_acc  = 0.0
    best_path     = os.path.join(args.output_dir, f'best_sparse_v3_jaad_step{args.step}.pth')
    min_loss      = 100.0
    min_loss_path = os.path.join(args.output_dir, f'min_loss_sparse_v3_jaad_step{args.step}.pth')

    print("Start Training (v3 JAAD)!")
    for epoch in range(args.epochs):
        train_loss = train_one_epoch(model, train_loader, optimizer, device, epoch, args.epochs)
        scheduler.step()
        val_loss, val_acc, val_prec, val_rec, val_f1 = evaluate_sparse_v3(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path',      default='/data/datasets/JAAD')
    parser.add_argument('--weights',        default='weights_v8/model_8_PIE_IL_step14_new.pth')
    parser.add_argument('--keypoints-dir',  default='/data/datasets/JAAD/keypoints_pid')
    parser.add_argument('--output-dir',     default='weights_sparse_v3_jaad')
